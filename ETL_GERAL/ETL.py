...
def main():
    ...
    raw_key_score = next(
        (
            k for k in raw_keys
            if f"empresa_{EMPRESA_ID}" in k
            and mac_address in k
            and "flights" not in k
            and "processos" not in k
            and k.endswith("/raw.csv")
        ),
        None
    )

    if not raw_key_score:
        print("RAW não encontrado para score.")


    df_score = ler_csv_s3(raw_key_score)
    print("Arquivo carregado:", raw_key_score)
    print("Colunas:", df_score.columns.tolist())
    ...
    if not raw_key_temp:
        print("RAW de temperatura não encontrado.")
        return
    ...
    if not raw_key_processos:
        print("RAW de processos não encontrado.")
        return
        ...
        for row in df.itertuples(index=False):
            ...
            if len(historico) >= 2:
                ...
                if diferenca_tempo > 0:
                    ...
                    tendencia_hora = diferenca_ram / diferenca_tempo
