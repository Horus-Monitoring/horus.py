...
def ler_csv(s3, key, bucket=None):
    if not key:
    ...
    try:
        ...
        if codigo in ("NoSuchKey", "404"):
            print(f"[S3] Arquivo nao encontrado: {key}")
            return pandas.DataFrame()
        raise
...
def limpar_dados(df):
    ...
    if "opensky_timestamp" in df.columns:
        df["opensky_timestamp"] = pandas.to_datetime(
            df["opensky_timestamp"],
            errors="coerce",
            utc=True,
        )
...
def gerar_alerta(row):
    alertas = []
    status_temp = str(row.get("status_temperatura", "")).upper()
    status_margem = str(row.get("status_margem", "")).upper()
    status_resfriamento = str(row.get("status_resfriamento", "")).upper()
    throttling = str(row.get("throttling", "")).upper()

    if status_temp in ("CRITICO", "ALTO"):
        alertas.append("Temperatura CPU elevada")
    if status_margem in ("CRITICO", "ALTO"):
        alertas.append("Margem termica critica")
    if status_resfriamento in ("CRITICO", "ALTO"):
        alertas.append("Resfriamento ineficiente")
    if throttling in ("SIM", "TRUE", "1", "YES"):
...
def processos_criticidade(cpu, ram_percent, latencia):
    cpu = safe_float(cpu)
    ram_percent = safe_float(ram_percent)
...
def top5cpu(df_processos):
    if df_processos.empty or "cpu" not in df_processos.columns:
        return {}

    df_top5 = df_processos.sort_values(
        "cpu",
        ascending=False,
    ).head(5)

    cpu5 = {}
    for i, (_, row) in enumerate(df_top5.iterrows(), start=1):
        cpu5[f"nome_cpu_{i}"] = row.get("nome")
        cpu5[f"cpu_{i}"] = safe_float(row.get("cpu"))

    return cpu5


def top5ram(df_processos):
    if df_processos.empty:
        return {}

    coluna_ram = "ram_percent" if "ram_percent" in df_processos.columns else "ram"
    if coluna_ram not in df_processos.columns:
        return {}
    ...
    ram5 = {}
    for i, (_, row) in enumerate(df_top5.iterrows(), start=1):
        ram5[f"nome_ram_{i}"] = row.get("nome")
        ram5[f"ram_{i}"] = safe_float(row.get(coluna_ram))

    return ram5


def processos_criticos(df_processos):
    if df_processos.empty or "criticidade" not in df_processos.columns:
        return {"totalCriticos": 0}
    return {"totalCriticos": int((df_processos["criticidade"] == "CRITICO").sum())}


def maior_latencia(df):
    if df.empty or "latencia_ms" not in df.columns:
        return {
            "nome": None,
            "latencia_ms": 0,
            "pid": None,
    ...
    idx = pandas.to_numeric(df["latencia_ms"], errors="coerce").fillna(0).idxmax()
    linha = df.loc[idx]

    return {
        "nome": linha.get("nome"),
        "latencia_ms": safe_float(linha.get("latencia_ms")),
        "pid": linha.get("pid"),
    }
...
def gerar_raw_criticos_4h(df):
    blocos_vazios = {b: 0 for b in [
        "atual", "0-2h59min", "3-5h59min", "6-8h59min", "9-11h59min",
        "12-14h59min", "15-17h59min", "18-20h59min", "21-23h59min",
    ]}
    if df.empty or "timestamp" not in df.columns or "criticidade" not in df.columns:
        return blocos_vazios

    df = df.copy()
    df["timestamp"] = pandas.to_datetime(df["timestamp"], errors="coerce", utc=True)
    agora = pandas.Timestamp.now(tz="UTC")
    df["horas_atras"] = (agora - df["timestamp"]).dt.total_seconds() / 3600
    df_criticos = df[df["criticidade"] == "CRITICO"].copy()
    ...
    df_criticos["bloco"] = df_criticos["horas_atras"].apply(bucket)
    resultado = df_criticos["bloco"].value_counts().to_dict()
    return {b: int(resultado.get(b, 0)) for b in blocos_vazios}
...
def contar_criticos(df):
    if df.empty or "criticidade" not in df.columns:
        return {
            "cpu": 0,
            "ram": 0,
            "latencia": 0,
    ...
    criticos = df[df["criticidade"] == "CRITICO"]

    cpu = (serie_numerica(criticos, "cpu") >= CPU_CRITICA).sum()
    ram = (serie_numerica(criticos, "ram_percent") > RAM_CRITICA_PERCENT).sum()
    latencia = (serie_numerica(criticos, "latencia_ms") > LATENCIA_CRITICA).sum()

    return {
        "cpu": int(cpu),
        "ram": int(ram),
        "latencia": int(latencia),
        "total": int(len(criticos)),
    }
...
def gerar_mensagem(metrica, nivel, previsao, limite):
    ...
    mensagens = {
        "CPU": {
            "baixa": f"CPU prevista em {pct}% do limite - Leve aumento na carga de processamento. Monitore a tendencia.",
            "media": f"CPU prevista em {pct}% do limite - Carga de processamento em elevacao. Verifique rotinas de calculo de rotas e separacao de aeronaves em execucao.",
            "alta": f"CPU prevista em {pct}% do limite - Processamento de dados de radar pode ser impactado. Considere redistribuir carga entre os nos do Sagitario.",
            "critico": f"CPU prevista em {pct}% do limite - Risco de atraso no processamento de dados de voo. Notifique um analista responsavel imediatamente.",
        },
        "RAM": {
            "baixa": f"RAM prevista em {pct}% do limite - Leve crescimento no consumo de memoria. Monitore a tendencia.",
            "media": f"RAM prevista em {pct}% do limite - Consumo de memoria crescente. Verifique buffers de dados de radar e faixas de voo ativas.",
            "alta": f"RAM prevista em {pct}% do limite - Risco de degradacao no gerenciamento de planos de voo. Verifique processos de correlacao de pistas.",
            "critico": f"RAM prevista em {pct}% do limite - Risco de falha no rastreamento de aeronaves. Reinicie processos nao essenciais e acione o sistema de contingencia do Sagitario.",
        },
        "DISCO": {
            "baixa": f"Disco previsto em {pct}% do limite - Leve crescimento no uso de armazenamento. Monitore a tendencia.",
            "media": f"Disco previsto em {pct}% do limite - Crescimento no volume de logs operacionais. Verifique retencao de gravacoes de voz e registros de radar.",
            "alta": f"Disco previsto em {pct}% do limite - Armazenamento de dados de voo pode ser comprometido. Realize purga de arquivos temporarios e logs antigos.",
            "critico": f"Disco previsto em {pct}% do limite - Risco de interrupcao no registro de dados operacionais. Arquive ou remova gravacoes antigas imediatamente e acione o suporte tecnico.",
        },
    }
        ...
        for metrica in ("cpu", "ram", "disco"):
            ...
            if nivel_atual in ("CRITICO", "ALTO"):
                alertas.append({
                    ...
                    "mensagem": gerar_mensagem(metrica.upper(), nivel_atual, recentes[-1], limite),
                })
                continue
            ...
            if a > 0 and nivel_p > 0.60 and nivel_p > atual:
                ...
                alertas.append({
                    ...
                    "mensagem": gerar_mensagem(metrica.upper(), nivel_cl, previsao, limite),
                })
...
def executar_pipeline_gestor(s3, bucket, empresa_id, mac_address, agora):
    ...
    salvar_s3_unificado(
        s3,
        montar_path("client", empresa_id, mac_address, "dashboard_gestor"),
        resultado,
...
def handler(event, context):
    ...
    raw_key = chave_raw_metrics(key_recebida, empresa_id, mac_address)
    flights_key = chave_raw_flights(key_recebida, empresa_id, mac_address)
    ...
    df = limpar_dados(ler_csv(s3, raw_key, bucket=bucket))
    ...
    print("\n=== PIPELINE PROCESSOS ===")
    if not df.empty:
        df_processos = processos_tratados_s3(df)
    ...
    df_flights = limpar_voos(ler_csv(s3, flights_key, bucket=bucket))
    ...
    executar_pipeline_gestor(s3, bucket, empresa_id, mac_address, agora)
